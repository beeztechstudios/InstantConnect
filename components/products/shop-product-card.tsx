    ...
    <div className="group">
      <Link href={`/product/${product.slug}`}>
        ...
        >
          ...
          {product.is_featured && (
            <span
              className="absolute left-2 top-2 sm:left-3 sm:top-3 rounded-full px-2 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-xs font-medium text-white z-10"
          ...
          <button
            ...
            className="lg:hidden absolute bottom-3 right-3 flex h-10 w-10 items-center justify-center rounded-full shadow-lg text-white z-10 active:scale-95 transition-transform"
            style={{ backgroundColor: '#685BC7' }}
          >
          ...
          >
                ...
                {images.map((_, index) => (
                  <button
                    ...
                    className={`h-2 w-2 rounded-full transition-colors ${
                      index === currentImageIndex ? 'bg-zinc-800' : 'bg-zinc-300'
                    }`}
                  />
                ))}
              </div>
            ...
            <button
              onClick={handleAddToCart}
              className="rounded-full px-6 py-2.5 text-sm font-semibold text-white shadow-lg hover:opacity-90 transition-opacity"
              style={{ backgroundColor: '#685BC7' }}
            >
              Add to cart
            </button>
