...
import { MapPin, Phone, Mail, Clock, Send, MessageSquare, Users, Building2 } from 'lucide-react'
...
const contactInfo = [
  {
    icon: MapPin,
    title: 'Visit Us',
    details: ['Siddharth Nagar, Opp. Miraj Malhar Apt.', 'New Bhupalpura, Udaipur', 'Rajasthan 313001'],
  },
  {
    icon: Phone,
    title: 'Call Us',
        ...
        <div className="mx-auto w-[95%]">
          <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
            {contactInfo.map((info) => (
              <div key={info.title} className="rounded-[10px] bg-white p-4 sm:p-5">
                <div className="flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-lg bg-violet-100">
                  <info.icon className="h-5 w-5 text-violet-600" />
                </div>
                <h3 className="mt-3 text-sm sm:text-base font-semibold text-zinc-900">{info.title}</h3>
                <div className="mt-1.5 space-y-0.5">
                  {info.details.map((detail, i) => (
                    <p key={i} className="text-xs sm:text-sm text-zinc-500">{detail}</p>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
            ...
            <div className="w-full lg:w-2/3">
              <div className="rounded-[10px] bg-white p-5 sm:p-6 lg:p-8">
                <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-zinc-900">Send us a Message</h2>
                    ...
                    {inquiryTypes.map((type) => (
                      ...
                      >
                        <type.icon className={`h-4 w-4 sm:h-5 sm:w-5 ${
                          selectedType === type.value ? 'text-violet-600' : 'text-zinc-400'
                        }`} />
                        <span className={`text-[10px] sm:text-xs font-medium ${
                          selectedType === type.value ? 'text-violet-700' : 'text-zinc-600'
                        }`}>
                          {type.label}
                        </span>
                      </button>
                ...
                <form onSubmit={handleSubmit} className="mt-5 sm:mt-6 space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input
                        ...
                        required
                        className="w-full rounded-lg border border-zinc-200 px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm focus:border-zinc-400 focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="mb-1 block text-xs sm:text-sm font-medium text-zinc-700">
                  ...
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-lg bg-zinc-900 px-5 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold text-white hover:bg-zinc-800 disabled:opacity-50"
                  >
                    {isSubmitting ? (
                      'Sending...'
                    ) : (
            ...
            <div className="w-full lg:w-1/3 space-y-4">
              ...
              <div className="rounded-[10px] bg-white p-5 sm:p-6">
                <h3 className="text-sm sm:text-base font-semibold text-zinc-900">Quick Links</h3>
                <div className="mt-3 space-y-2">
                  <Link
                    href="/faqs"
                    className="flex items-center gap-3 rounded-lg bg-zinc-50 p-3 text-xs sm:text-sm font-medium text-zinc-700 hover:bg-zinc-100"
                  >
              ...
              <div className="rounded-[10px] p-5 sm:p-6" style={{ backgroundColor: '#F5A623' }}>
                <h3 className="text-sm sm:text-base font-semibold text-black">Need Bulk Orders?</h3>
                <p className="mt-2 text-xs sm:text-sm text-black/70">
                  Get special pricing for orders of 50+ units. Our team will create a custom quote.
                </p>
                <button
                  onClick={() => setSelectedType('bulk')}
                  className="mt-4 w-full rounded-lg bg-black/20 px-4 py-2.5 text-xs sm:text-sm font-semibold text-black hover:bg-black/30"
                >
                  Get Bulk Quote
                </button>
              </div>
              ...
              <div className="rounded-[10px] bg-zinc-900 p-5 sm:p-6 text-white">
                <h3 className="text-sm sm:text-base font-semibold">Response Time</h3>
                <div className="mt-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs sm:text-sm text-zinc-400">Email</span>
                    <span className="text-xs sm:text-sm font-medium">Within 24 hrs</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs sm:text-sm text-zinc-400">Phone</span>
                    <span className="text-xs sm:text-sm font-medium">Instant</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs sm:text-sm text-zinc-400">WhatsApp</span>
                    <span className="text-xs sm:text-sm font-medium">Within 2 hrs</span>
                  </div>
                </div>
              </div>
            </div>
