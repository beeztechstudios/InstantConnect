...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      const [
        ...
        supabase
          .from('orders')
          .select('id, order_number, status, total, created_at, customers(first_name, last_name, email)')
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Alert Banner */}
      {(stats.pendingOrders > 0 || stats.pendingDemos > 0 || stats.lowStockProducts > 0) && (
        <div className="flex flex-wrap gap-3">
          ...
          {stats.pendingDemos > 0 && (
            <Link
              ...
              className="flex items-center gap-2 rounded-lg bg-violet-50 px-4 py-2 text-sm font-medium text-violet-700 transition-colors hover:bg-violet-100"
            >
              <Calendar className="h-4 w-4" />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {/* Revenue Card */}
        <div className="rounded-xl border border-zinc-200 bg-gradient-to-br from-teal-500 to-teal-600 p-5 text-white">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-teal-100">Total Revenue</span>
            <div className="rounded-lg bg-white/20 p-2">
              <IndianRupee className="h-5 w-5" />
            </div>
          </div>
          <p className="mt-3 text-3xl font-bold">{formatPrice(stats.totalRevenue)}</p>
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-5">
          ...
          <p className="mt-3 text-3xl font-bold text-zinc-900">{stats.totalOrders}</p>
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-zinc-500">Total Customers</span>
            <div className="rounded-lg bg-blue-100 p-2">
              <Users className="h-5 w-5 text-blue-600" />
            </div>
          </div>
          <p className="mt-3 text-3xl font-bold text-zinc-900">{stats.totalCustomers}</p>
          <div className="mt-2 text-sm text-zinc-500">Registered users</div>
        </div>
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-5">
          ...
          <p className="mt-3 text-3xl font-bold text-zinc-900">{stats.totalProducts}</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Link
          ...
          className="group flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-4 transition-all hover:border-teal-200 hover:shadow-md"
        >
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-teal-100 text-teal-600 transition-colors group-hover:bg-teal-500 group-hover:text-white">
        ...
        <Link
          ...
          className="group flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-4 transition-all hover:border-violet-200 hover:shadow-md"
        >
          ...
          <div>
            <p className="font-semibold text-zinc-900">Add Category</p>
            <p className="text-xs text-zinc-500">Organize products</p>
          </div>
        </Link>

        <Link
          href="/admin/coupons/new"
          className="group flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-4 transition-all hover:border-orange-200 hover:shadow-md"
        >
          ...
          <div>
            <p className="font-semibold text-zinc-900">Create Coupon</p>
            <p className="text-xs text-zinc-500">Add discount code</p>
          </div>
        </Link>

        <Link
          href="/admin/testimonials/new"
          className="group flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-4 transition-all hover:border-amber-200 hover:shadow-md"
        >
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-amber-100 text-amber-600 transition-colors group-hover:bg-amber-500 group-hover:text-white">
            <Star className="h-5 w-5" />
          </div>
          <div>
            <p className="font-semibold text-zinc-900">Add Review</p>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-zinc-100 text-sm font-medium text-zinc-600">
                      {order.customers?.first_name?.charAt(0) || 'G'}
                    </div>
                    <div>
                      ...
                      <p className="text-sm text-zinc-500">{order.order_number}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold text-zinc-900">{formatPrice(order.total)}</p>
                    <span
                      className={cn(
          ...
          <div className="rounded-xl border border-zinc-200 bg-white">
            <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-zinc-900">Demo Requests</h2>
              </div>
              <Link
                href="/admin/demo-bookings"
              ...
              {recentDemos.length > 0 ? (
                recentDemos.slice(0, 4).map((demo) => (
                  <div key={demo.id} className="px-5 py-3">
                    <div className="flex items-center justify-between">
                      <p className="font-medium text-zinc-900">{demo.name}</p>
                      <span
                        className={cn(
                ...
                <div className="px-5 py-8 text-center">
                  ...
                  <p className="mt-2 text-sm text-zinc-500">No demo requests</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-zinc-200 bg-white">
            <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-zinc-900">Low Stock</h2>
              </div>
            </div>
            <div className="divide-y divide-zinc-100">
              {lowStockProducts.length > 0 ? (
                ...
                <div className="px-5 py-8 text-center">
                  ...
                  <p className="mt-2 text-sm text-zinc-500">All stocked up!</p>
                </div>
              )}
            </div>
          </div>
